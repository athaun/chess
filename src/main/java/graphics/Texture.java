...
public class Texture {
	 ...
	 * Recommended to use Assets.loadTexture instead of calling this function
	 *
	 * @param image ByteBuffer containing pixel data
	 */
	public Texture (ByteBuffer image, int width, int height, int channels) {
		...
		if (image != null) {
			...
			if (channels == 3) {
				...
				assert false : "[ERROR] Graphics.Texture - Unknown number of channels \"" + channels + "\".";
			}
		} else {
			assert false : "[ERROR] Graphics.Texture - Could not load image \"" + filepath + "\".";
		}
	 ...
	 */
	public ByteBuffer loadImageAsByteBuffer (String path) {
		ByteBuffer image;
	...
	public static int[][][] getPixels (BufferedImage image) {
		...
		for (int x = 0; x < width; x ++) {
			for (int y = 0; y < height; y ++) {
				...
				if (hasAlphaChannel)
					colors[3] = (pixels[pos++] & 0xff); // Alpha
				colors[2] = (pixels[pos++] & 0xFF); // Blue
				colors[1] = (pixels[pos++] & 0xFF); // Green
				colors[0] = (pixels[pos++] & 0xFF); // Red
				result[x][y] = colors;
			}
		}
		return result;
	...
	public static ByteBuffer createByteBuffer (int[][][] pixels) {
		ByteBuffer buffer = BufferUtils.createByteBuffer(pixels.length * pixels[0].length * 4); //4 for RGBA, 3 for RGB

		for (int y = pixels[0].length - 1; y >= 0 ; y --) {
			for (int x = pixels.length - 1; x >= 0 ; x --) {
				int[] pixel = pixels[x][y];
				buffer.put((byte) pixel[0]); // Red component
				buffer.put((byte) pixel[1]); // Green component
				buffer.put((byte) pixel[2]); // Blue component
				buffer.put((byte) pixel[3]); // Alpha component. Only for RGBA
			}
		}
		return buffer.flip();
	}
